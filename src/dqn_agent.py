...
class Agent():
    ...
    def step(self,state,action,reward,next_state,done):
        ...
        if self.t_step == 0:
            #if samples are there then learn on a subset
            if len(self.memory) > BATCH_SIZE:
                experiences = self.memory.sample()
                self.learn(experiences,GAMMA)
    
    def act(self,state,eps = 0.):
        """Returns action for given state as per policy"""
    ...
    def learn(self,experiences,gamma):
        """Uses double QN to update parameters"""      
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm(self.qnetwork_local.parameters(),1.0)
